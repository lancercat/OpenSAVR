...
from neko_2020nocr.dan.methods_pami.pami_osds_paths  import get_lsvtK_path,get_ctwK_path,\
    get_mlt_chlatK_path,get_artK_path,get_rctwK_path,get_mltjp_path,get_monkey_path,get_mltkr_path,get_mltbe_path,get_mlthn_path
...
def get_kr_te_meta(root):
    ...
    return  temeta;
def get_be_te_meta(root):
    temeta = os.path.join(root,"dicts","dabbemlt.pt");
    return  temeta;

def get_hn_te_meta(root):
    temeta = os.path.join(root,"dicts","dabhnmlt.pt");
    return  temeta;
...
def get_eval_be_color(root,maxT,hw=[32,128]):
    teroot= get_mltbe_path(root);
    return {
        "dict_dir":None,
        "case_sensitive": False,
        "te_case_sensitive": False,
            "datasets":{
                "BE_lang": get_eval_word_color_core(teroot,maxT,hw),
            }
        }
def get_eval_hn_color(root,maxT,hw=[32,128]):
    teroot= get_mlthn_path(root);
    return {
        "dict_dir":None,
        "case_sensitive": False,
        "te_case_sensitive": False,
            "datasets":{
                "HN_lang": get_eval_word_color_core(teroot,maxT,hw),
            }
        }
