...
class neko_abstract_routine:
    ...
    def fpbp_impl(this, input_dict, modular_dict, logger_dict, nEpoch, batch_idx, device):
        loss= this.fp_impl(input_dict, modular_dict, logger_dict, nEpoch, batch_idx, device);
        this.bp_impl(loss);
        pass;
    def fpbp_amp_impl(this, input_dict, modular_dict, logger_dict, nEpoch, batch_idx, device):
        with torch.cuda.amp.autocast():
            loss= this.fp_impl(input_dict, modular_dict, logger_dict, nEpoch, batch_idx, device);
        this.bp_impl(loss);
        pass;

    def fp(this, input_dict, modular_dict, nEpoch, batch_idx, device):
        idict, mdict = this.grab_modules(input_dict, modular_dict);
        loss = this.fp_impl(idict, mdict, this.logger_dict, nEpoch, batch_idx, device);
        if (batch_idx % this.log_each == 0):
            this.log_show();
        return loss;

    def fpbp(this,input_dict,modular_dict,nEpoch,batch_idx,device="cuda"):
        idict,mdict=this.grab_modules(input_dict,modular_dict);
        ...
        ret= this.fpbp_impl(idict, mdict, this.logger_dict, nEpoch, batch_idx, device);
        if (batch_idx % this.log_each == 0):
            this.log_show();
        return ret;
    def fpbp_amp(this,input_dict,modular_dict,nEpoch,batch_idx):
        idict,mdict=this.grab_modules(input_dict,modular_dict);
        ret= this.fpbp_amp_impl(idict, mdict, this.logger_dict, nEpoch, batch_idx, device);
        if (batch_idx % this.log_each == 0):
            this.log_show();
        return ret;
...
class neko_abstract_eval_routine:
    ...
    def interpret_mods(this,modular_dict):
        ...
        for k in this.mod_cvt_dict:
            if (type(this.mod_cvt_dict[k]) is list):
                ...
                if (this.mod_cvt_dict[k] == "NEP_skipped_NEP"):
                    mdict[k] = None;
                else:
    ...
    def test_topk_impl(this,input_dict,modular_dict,logger_dict,k):
        pass;
    ...
    def test_topk(this, input_dict, modular_dict,k, override=None, vdbg=None):
        idict, mdict = this.grab_modules(input_dict, modular_dict);
        if (not (vdbg is None)):
            idict["vdbg"] = vdbg;
            try:
        ...
        if (override is not None):
            for i in override:
                mdict[i] = modular_dict[i]
        return this.test_topk_impl(idict, mdict, this.logger_dict,k);
