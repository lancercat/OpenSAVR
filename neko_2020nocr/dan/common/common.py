...
def flatten_label(target,EOSlen=1,length=None,device="cpu"):
    label_flatten = []
    label_length = []
    ...
    label_flatten = torch.LongTensor(label_flatten,device=device)
    if (length is None):
        label_length = torch.IntTensor(label_length,device=device)
    else:
        label_length=torch.IntTensor(length,device=device)
    return (label_flatten, label_length)
def flatten_label_idx(target,EOSlen=1,length=None,device="cpu"):
    label_flatten = []
    label_length = []
    src=[];
    for i in range(0, target.size()[0]):
    ...
    if (length is None):
        ...
        label_length = torch.IntTensor(length,device=device)
    src=torch.LongTensor(src,device=device);
