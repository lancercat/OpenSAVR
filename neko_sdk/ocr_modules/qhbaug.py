...
def qhbwarp(img, ang, use_tia=True, prob=0.4,rng=None):
    """
    warp
    """
    # just in case random messes up.
    if rng is None:
        rng=random;
    h, w, _ = img.shape
    ...
    if config.distort:   #扭曲
        ...
        if rng.random() <= prob and img_height >= 20 and img_width >= 20:
            new_img = tia_distort(new_img, rng.randint(3, 6))

    if config.stretch:   #拉伸
        img_height, img_width = img.shape[0:2]
        if rng.random() <= prob and img_height >= 20 and img_width >= 20:
            new_img = tia_stretch(new_img, rng.randint(3, 6))
